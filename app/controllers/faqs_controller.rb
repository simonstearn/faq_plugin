...
#  before_filter :authorize, :except => :show
  ...
  before_filter :find_faq, :only => [:edit, :copy, :destroy]
  before_filter :materialise, :only => :show
  ...
  def index
    ...
    respond_to do |format|
      format.html { render :template => 'faq/index.html.erb', :layout => !request.xhr? }
  ...
  def list_invalid_faqs
    ...
    render(:template => 'faq/list_invalid_faqs.html.erb', :layout => !request.xhr?)
  end
  ...
  def show
    ...
    respond_to do |format|
      format.html { render :template => 'faq/show.html.erb', :layout => !request.xhr? }
  ...
  def new
    ...
    if request.get? || request.xhr?
      ...
      if @faq.save
        ...
        redirect_to :controller => 'faqs', :action => 'show', :id => @project, :faq_id => @faq
        return
      end		
    end
  ...
  def copy
    ...
    if request.post?
      ...
      flash[:notice] = l(:notice_successful_update)
      redirect_to :controller => 'faqs', :action => 'index', :id => @project
      return
    end
    if request.xhr?
      render :template => 'faq/copy', :layout => false
    else
      render :template => 'faq/copy'
    end
  end
...
private
  ...
  def find_faq
    puts "find faq"
    @faq = Faq.find(:first, :conditions => "project_id = #{@project.id} and id = #{params[:faq_id]}") unless @faq
  ...
  def materialise
    # Hack
    puts "materialise"
    @faq = Faq.find_by_id(params[:faq_id]) unless @faq
    @faq = Faq.find_by_id(params[:id]) unless @faq

    puts @faq.id.to_s + " " + @faq.question
    puts "Project id:" + @faq.project_id.to_s
    @project = Project.find_by_id(@faq.project_id)
    puts @project.name
    render_404 if (@faq.nil? or @project.nil?)
  rescue
    render_404
  end
  ...
  def faq_to_pdf
    ...
    pdf.Cell(200,5, 'FAQ export to PDF')
  ...
  def faqs_to_pdf
    ...
    pdf.Line(pdf.GetX, pdf.GetY, 100, pdf.GetY)
    pdf.SetFontStyle('I',8)
    pdf.Cell(200,5, 'Auto generated faq-list by FAQ plugin for chiliproject.')
